...
const TicketDetail: React.FC<TicketDetailProps> = ({ ticketId, isAdmin: propIsAdmin }) => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { user, hasRole } = useAuth();

  // 使用传入的isAdmin属性，如果没有则使用上下文中的hasRole函数
  const isAdminUser = propIsAdmin !== undefined ? propIsAdmin : hasRole('admin');
  ...
  const fetchTicket = async () => {
    try {
      ...
      const response = await api.getTicketById(Number(ticketIdToUse));
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      await api.updateTicket(Number(id), values);
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      await apiAny.addTicketComment(id, { content: commentValue });
