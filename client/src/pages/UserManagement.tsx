...
const UserManagement: React.FC = () => {
  ...
  // 打开修改密码对话框
  const showPasswordModal = (user: any) => {
    setCurrentEditUser(user);
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      await api.updateUser(currentEditUser.id, values);
  ...
  const handlePasswordSubmit = async () => {
    try {
      ...
      await apiAny.updatePassword(currentEditUser.id, values);
  ...
  const approveUser = async (userId: number) => {
    try {
      await api.updateUser(userId, { status: 'active' } as any);
  ...
  const disableUser = async (userId: number) => {
    try {
      await api.updateUser(userId, { status: 'disabled' } as any);
  ...
  const deleteUser = async (userId: number) => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const apiAny = api as any;
      await apiAny.deleteUser(userId);
