...
const ProjectDetail: React.FC = () => {
  ...
  const fetchProject = async () => {
    try {
      setLoading(true);
      const response = await api.getProjectById(Number(id));
  ...
  const handleEditProjectSubmit = async () => {
    try {
      ...
      await api.updateProject(Number(id), values);
  ...
  // 打开创建/编辑工作项模态框
  const showWorkItemModal = (workItem: any = null) => {
