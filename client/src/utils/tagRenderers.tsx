...
// 优先级标签渲染
export const renderPriorityTag = (priority: string): React.ReactElement => {
  let color = '';
  switch (priority) {
    case '紧急':
...
// 状态标签渲染
export const renderStatusTag = (status: string): React.ReactElement => {
  let className = '';
  switch (status) {
    case '待处理':
...
// 类型标签渲染
export const renderTypeTag = (type: string): React.ReactElement => {
  let className = '';
  switch (type) {
    case '规划':
